...
const static vec3_m_t G = 6.6743e-2;   // gravitational constant in m^3/(kg s^2)
...
void compute_forces_newtonian(Particle particles[], int n, int start, int end, double dt)
{
    ...
    for (i = start; i < end; i++)
    {
        ...
        for (j = 0; j < n; j++)
        {
            ...
            particles[i].vel = vec3_add(particles[i].vel, vec3_mul(dt, vec3_div(vec3_mul(force_mag, r), particles[i].mass * dist)));
        }
    }
}
...
void compute_forces_schwarzschild_GR(Particle particles[], int n, int start, int end, double dt)
{
    ...
    for (i = start; i < end; i++)
    {
        ...
        for (j = 0; j < n; j++)
        {
            ...
            particles[i].vel = vec3_add(particles[i].vel, vec3_mul(dt, vec3_div(vec3_mul(force_mag, r), particles[i].mass * c * c * force_factor)));
        }
